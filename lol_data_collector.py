...
''' Funkcja do pobrania PUUID na podstawie nicku i tagu '''
...
''' Funkcja do pobrania szczegółowych danych meczu '''
...
''' Funkcja do pobrania danych z timeline meczu '''
...
''' Funkcja do zarządzania limitami zapytań '''
def wait_for_rate_limit():
    now = time.time()
    
    ''' Usuń zapytania starsze niż 2 minuty '''
    while request_timestamps and request_timestamps[0] < now - 120:
        request_timestamps.popleft()

    ''' Jeśli przekroczono 100 zapytań w ciągu ostatnich 2 minut, poczekaj '''
    ...
    ''' Jeśli przekroczono 20 zapytań w ciągu ostatniej sekundy, poczekaj '''
...
''' Funkcja filtrująca dane timeline '''
def extract_data_from_timeline(timeline_data, match_data):
    ...
    for participant_id, participant_data in participant_frames.items():
        ...
        # Statystyki dla pojedynczych graczy
...
def main():
    ...
    ''' Pobranie PUUID dla każdego gracza z listy '''
    ...
    ''' Pobieranie danych meczowych na podstawie PUUID '''
    for puuid in puuid_list:
        ...
        for match_id in match_ids:
            ...
            ''' Zbieranie wyników dla graczy i drużyn '''
            match_result = {
                "match_id": match_id,
